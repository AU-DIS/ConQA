...
class FastBLIP2ITCSearchEngine:
    '''
    BLIP2SearchEngine returns a list of candidates using a BLIP2 based similarity.
    Images are indexed using a BLIP2 Neural Network for getting the embeddings.
    Uses ITC
    '''
...
class FastBLIP2ITCRRSearchEngine:
    '''
    BLIP2SearchEngine returns a list of candidates using a BLIP2 based similarity.
    Images are indexed using a BLIP2 Neural Network for getting the embeddings.

    Uses ITC. Reranks the top 128 results using ITM. 
    '''
    ...
    def __process_batch(self, batch):
        with torch.no_grad(): #Do not need to track the gradients for inferences
            ...
            self.image_atts.append(image_atts.cpu())
            self.query_tokens.append(query_tokens.cpu())
            self.image_feats.append(image_feats)
        pass
    ...
    def index(self, images_path: Dict[int, Tuple[str, str]], batch_size=50) -> None:
        ...
        self.image_atts = torch.concat(self.image_atts, dim=0).to(torch.float32)
        self.query_tokens = torch.concat(self.query_tokens, dim=0).to(torch.float32)
        self.image_feats = torch.concat(self.image_feats, dim=0).to(torch.float32).to(self.inference_device)
        self.trained = True
        self.model = self.model.to(self.inference_device)
        pass
    ...
    def load(self, path: str) -> None:
        ...
        self.image_atts = param_dict['image_atts']
        self.query_tokens = param_dict['query_tokens']
        self.image_feats = param_dict['image_feats']
        self.image_idx = idxs
        self.trained = True
        self.model = self.model.to(self.inference_device)
        pass
    ...
    def save(self, path: str) -> None:
        ...
        torch.save({'image_atts': self.image_atts,
                    ...
                    'image_feats': self.image_feats}, tensor_path)
        with open(index_path, 'wb') as f:
            pickle.dump(self.image_idx, f)
        pass
    ...
    def rerank(self, text, ids: List[int]) -> Tuple[List[int], List[float]]:
        ...
        with torch.no_grad(): #Do not need to track the gradients for inferences
            ...
            t_ids = [self.idx_map[i] for i in ids]
            query_tokens = self.query_tokens[t_ids, ...]
            image_embeds = self.image_embeds[t_ids, ...]
            image_embeds = image_embeds.to(self.inference_device)
            image_atts = self.image_atts[t_ids, ...] 
            itm_scores = self._batch_query_processing(text, query_tokens, image_embeds, image_atts)

            arg_sorted = itm_scores.argsort(descending=True).numpy()
            similitud = itm_scores.cpu().numpy()

        return [ids[idx] for idx in arg_sorted], similitud[arg_sorted].tolist()
    ...
    def search_text(self, sentences: str, k: Optional[int]=None) -> Tuple[List[int], List[float]]:
        ...
        with torch.no_grad(): #Do not need to track the gradients for inferences
            ...
            similitud = torch.max(self.image_feats @ text_feat.T, dim=1)[0][:, 0] #CHECK!
            arg_sorted = similitud.argsort(descending=True).cpu().numpy()
            similitud = similitud.cpu().numpy()
        if k is not None:
            arg_sorted = arg_sorted[:k]
