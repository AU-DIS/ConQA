...
class NAAFSearchEngine:
    '''
    NAAFSearchEngine returns a similarities between all the sentences and all the images calculated by NAAF method.
    '''
    ...
    def search_text(self, sentences: int, k: Optional[int]=None) -> Tuple[List[int], List[float]]:      
        ...
        with torch.no_grad(): #Do not need to track the gradients for inferences
            # sentences_features = self.caption_embeddings[sentences]
