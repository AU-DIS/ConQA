...
def evalrank(model_path, experiment, data_path=None, split='dev', fold5=False):
    """
    ...
    vocab = deserialize_vocab(os.path.join('./vocab/', '%s_vocab.json' % opt.data_name))
    opt.vocab_size = len(vocab)

    # construct model
    model = SGRAF(opt)
    model_parameters_txt = filter(lambda p: p.requires_grad, model.txt_enc.parameters())
    print('# txt enc trainable param',  sum([np.prod(p.size()) for p in model_parameters_txt]))
    model_parameters_img = filter(lambda p: p.requires_grad, model.img_enc.parameters())
    print('# txt enc trainable param',  sum([np.prod(p.size()) for p in model_parameters_img]))
    model_parameters_sim = filter(lambda p: p.requires_grad, model.sim_enc.parameters())
    print('# txt enc trainable param',  sum([np.prod(p.size()) for p in model_parameters_sim]))
    # load model state
    model.load_state_dict(checkpoint['model'])

    print('Loading dataset')
    data_loader = get_test_loader(split, experiment, opt.data_name, vocab,
    ...
    if experiment != 'coco5k':
        if img_embs.shape[0] > 6390:
            img_embs = img_embs[:6390]
    ...
    if not fold5:
        ...
        np.savez(f'../sims/sgraf_{experiment}_sims.npz', data=sims)
...
def i2t(images, captions, caplens, sims, npts=None, return_ranks=False):
    ...
    print('std r@1 i2t: ', np.std(ranks<1) )
    print('std r@5 i2t: ', np.std(ranks<5) )
    print('std r@10 i2t: ', np.std(ranks<10) )
    medr = np.floor(np.median(ranks)) + 1
    meanr = ranks.mean() + 1
    print('MRR i2t:', np.mean([1/(i+1) for i in ranks]), np.std([1/(i+1) for i in ranks]))
    if return_ranks:
...
def t2i(images, captions, caplens, sims, npts=None, return_ranks=False):
    ...
    medr = np.floor(np.median(ranks)) + 1
    meanr = ranks.mean() + 1
    print('std r@1 t2i: ', np.std(ranks<1) )
    print('std r@5 t2i: ', np.std(ranks<5) )
    print('std r@10 t2i: ', np.std(ranks<10) )
    print('MRR t2i:', np.mean([1/(i+1) for i in ranks]), np.std([1/(i+1) for i in ranks]))
    if return_ranks:
        return (r1, r5, r10, medr, meanr), (ranks, top1)
    else:
        return (r1, r5, r10, medr, meanr)
...
if __name__ == '__main__':
    ...
    if args.experiment == 'coco5k':
        evalrank("./MSCOCO_SGRAF/coco_SGR/model_best.pth.tar", args.experiment, 
             data_path="../small-models-dataset", split="test", fold5=False)
    else:
        evalrank("./Flickr30K_SGRAF/f30k_SGR/model_best.pth.tar", args.experiment, 
                data_path="../small-models-dataset", split="test", fold5=False)
