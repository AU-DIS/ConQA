...
class STCLIPSearchEngine:
    ...
    def __init__(self, model, device=None, 
        ...
        self.model = SentenceTransformer(f'clip-{model.replace("/", "-")}')
    ...
    def __process_batch(self, batch):
        with torch.no_grad(): 
            image_features = self.model.encode(batch)
    ...
    def index(self, images_path: Dict[int, Tuple[str, str]], batch_size=100) -> None:
        ...
        for k, (zip_path, internal_path) in images_path.items():
            ...
            image_data = Image.open(internal_file).convert('RGB')
            internal_file.close()
        ...
        if len(images_batch) != 0:
            ...
            p_bar.update(len(images_batch))
        p_bar.close()
        self.image_embeddings = torch.from_numpy(np.concatenate(self.image_embeddings, axis=0)).to(torch.float32)
    ...
    def search_text(self, sentences: str, k: Optional[int]=None) -> Tuple[List[int], List[float]]:        
        with torch.no_grad(): #Do not need to track the gradients for inferences
            sentences_features = self.model.encode(sentences)[np.newaxis, :]
            sentences_features = torch.from_numpy(sentences_features).to(self.device).to(torch.float32)
